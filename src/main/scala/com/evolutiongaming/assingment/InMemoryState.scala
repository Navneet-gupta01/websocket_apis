    ...
    case login(us, ps) =>
      ...
      if (filteredUser.size > 0)
        connected.find(_._1 === us).fold(
          (this.copy(connected = connected + (us ->
            (req.user, filteredUser.head.userType))),
            Seq(UserReqWrapper(req.user, login_successful(filteredUser.head.userType)))))(cu => {
          val oldConnectionUUID = cu._2._1
          (this.copy(connected = connected + (us ->
            (req.user, filteredUser.head.userType))),
            Seq(UserReqWrapper(req.user, login_successful(filteredUser.head.userType)), UserReqWrapper(oldConnectionUUID, AnotherSession("Disconnecting Current Session, Another Client LoggedIn"))))
        })
      else
